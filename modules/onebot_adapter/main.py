...
class OneBotAdapter:
    ...
    async def _action_send_msg(self, params, echo):
        ...
        ok, data = await self._do_send(sender, gid, uid, payload, image_bytes, self._find_msg_id(real_id))
    ...
    def convert_payload(self, payload: str | dict):
        ...
        if isinstance(payload, dict):
            ...
            if msg_type == 'markdown':
                ...
                if buttons := payload.get('keyboard'):
                    payload['buttons'] = buttons.get('rows') or []
                    del payload['keyboard']
                if 'markdown' in payload:
                    payload |= payload['markdown']
                    del payload['markdown']
                return payload | {'msg_type': MessageType.MSG_TYPE_MARKDOWN}
