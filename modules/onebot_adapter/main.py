...
class OneBotAdapter:
    ...
    async def start(self):
        ...
        self.log.info(
            f"配置: path={ws_path}, token={'***' if self.cfg['access_token'] else '(无)'}"
        )
    ...
    async def _action_send_msg(self, params, echo):
        ...
        payload, image_bytes = _parse_segments(params.get("message", ""))
        ...
        ok, data = await self._do_send(
            sender, gid, uid, payload, image_bytes, self._find_msg_id(real_id)
        )
        await self._log_msg(
    ...
    async def _do_send(
        ...
        kwargs = self.convert_payload(payload)
        fn = sender.send_to_group if group_id else sender.send_to_user
        ok, data, _ = await fn(target, msg_id=msg_id, **kwargs)
        return ok, data

    def convert_payload(self, payload: str | dict):
        ...
        if isinstance(payload, dict):
            ...
            if msg_type == "markdown":
                payload = payload.get("data", {}).get("data", {})
                return {"content": payload, "msg_type": MessageType.MSG_TYPE_MARKDOWN}
    ...
    async def _action_get_group_list(self, params, echo):
        groups = []
        for (_openid, id_type), qq_id in self.id_mapper._cache_fwd.items():
            if id_type == "group":
                groups.append(
                    {
                        "group_id": qq_id,
    ...
    def _get_ls(self, appid: str = ""):
        ...
        return self._log_services.get(aid) or next(
            iter(self._log_services.values()), None
        )
    ...
    async def _log_msg(self, msg_type, target_id, content, ok, resp_data):
        ...
        self._push_ws(
            user_id=uid, group_id=gid, content=content, is_bot=True, direction="send"
        )
