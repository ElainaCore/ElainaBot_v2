...
class ImageHosting:
    ...
    def initialize(self):
        ...
        status.append(
            f"B站={'✅' if bili_cfg.get('enabled') and bili_cfg.get('csrf_token') and bili_cfg.get('sessdata') else '❌'}"
        )
        status.append(
            f"QQ频道={'✅' if qq_cfg.get('enabled') and qq_cfg.get('channel_id') else '❌'}"
        )
        status.append(
            f"ChatGLM={'✅' if self._cfg.get('chatglm', {}).get('enabled') else '❌'}"
        )
        status.append(
            f"Ukaka={'✅' if self._cfg.get('ukaka', {}).get('enabled') else '❌'}"
        )
        status.append(
            f"星野={'✅' if self._cfg.get('xingye', {}).get('enabled') else '❌'}"
        )
        status.append(
            f"Nature={'✅' if self._cfg.get('nature', {}).get('enabled') else '❌'}"
        )
    ...
    def _upload_cos_sync(self, file_data, filename, user_id, custom_path):
        ...
        try:
            file_bytes = (
                file_data.getvalue() if isinstance(file_data, BytesIO) else file_data
            )
    ...
    async def upload_chatglm(self, image_data):
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            _executor, self._upload_chatglm_sync, image_data
        )
    ...
    async def upload_ukaka(self, image_data):
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            _executor, self._upload_signed_sync, image_data, "ukaka"
        )
    ...
    async def upload_xingye(self, image_data):
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            _executor, self._upload_signed_sync, image_data, "xingye"
        )
    ...
    async def upload_nature(self, image_data):
        ...
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(
            _executor, self._upload_nature_sync, image_data
        )
    ...
    def _upload_nature_sync(self, image_data):
        try:
            ...
            auth = (
                f"q-sign-algorithm=sha1&q-ak={sid}"
