...
class RedisPool:
    ...
    async def expire(self, key, seconds):
        return bool(
            await self._safe(
                "EXPIRE", self._client.expire(key, seconds), default=False, key=key
            )
        )
    ...
    async def expireat(self, key, when):
        ...
        return bool(
            await self._safe(
                "EXPIREAT", self._client.expireat(key, when), default=False, key=key
            )
        )
    ...
    async def incr(self, key, amount=1):
        return await self._safe(
            "INCR", self._client.incrby(key, amount), default=None, key=key
        )

    async def decr(self, key, amount=1):
        return await self._safe(
            "DECR", self._client.decrby(key, amount), default=None, key=key
        )
    ...
    async def hget(self, name, key, default=None):
        if not self.is_available():
            return default
        v = await self._safe(
            "HGET", self._client.hget(name, key), default=default, key=f"{name}.{key}"
        )
        return v if v is not None else default
    ...
    async def hdel(self, name, *keys):
        if not keys:
            return 0
        return await self._safe(
            "HDEL", self._client.hdel(name, *keys), default=0, key=name
        )

    async def hgetall(self, name):
        return await self._safe(
            "HGETALL", self._client.hgetall(name), default={}, key=name
        )
    ...
    async def lpush(self, name, *values):
        if not values:
            return 0
        return await self._safe(
            "LPUSH", self._client.lpush(name, *values), default=0, key=name
        )
    ...
    async def rpush(self, name, *values):
        if not values:
            return 0
        return await self._safe(
            "RPUSH", self._client.rpush(name, *values), default=0, key=name
        )

    async def lpop(self, name, count=None):
        return await self._safe(
            "LPOP", self._client.lpop(name, count), default=None, key=name
        )

    async def rpop(self, name, count=None):
        return await self._safe(
            "RPOP", self._client.rpop(name, count), default=None, key=name
        )

    async def lrange(self, name, start, end):
        return await self._safe(
            "LRANGE", self._client.lrange(name, start, end), default=[], key=name
        )
    ...
    async def sadd(self, name, *values):
        if not values:
            return 0
        return await self._safe(
            "SADD", self._client.sadd(name, *values), default=0, key=name
        )
    ...
    async def srem(self, name, *values):
        if not values:
            return 0
        return await self._safe(
            "SREM", self._client.srem(name, *values), default=0, key=name
        )

    async def smembers(self, name):
        return await self._safe(
            "SMEMBERS", self._client.smembers(name), default=set(), key=name
        )
    ...
    async def zadd(self, name, mapping, nx=False, xx=False):
        return await self._safe(
            "ZADD", self._client.zadd(name, mapping, nx=nx, xx=xx), default=0, key=name
        )
    ...
    async def zrem(self, name, *values):
        if not values:
            return 0
        return await self._safe(
            "ZREM", self._client.zrem(name, *values), default=0, key=name
        )
    ...
    async def zscore(self, name, value):
        return await self._safe(
            "ZSCORE", self._client.zscore(name, value), default=None, key=name
        )

    async def zincrby(self, name, amount, value):
        return await self._safe(
            "ZINCRBY", self._client.zincrby(name, amount, value), default=None, key=name
        )
