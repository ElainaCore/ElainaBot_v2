...
class ReceiveEventHandler:
    ...
    async def _on_event(self, event):
        ...
        from core.bot.event import (
            ...
            MESSAGE_TYPES, SILENT_TYPES, _EventDedup,
        )
        ...
        if et in SILENT_TYPES:
            ...
            bot.log_service.add_sync('lifecycle', {
                'type': et, 'user_id': event.user_id or '',
                'group_id': event.group_id or '', 'extra': raw_json,
            })
            self._push_web_log('event', {
                'appid': appid, 'event_type': et, 'content': raw_json,
                'raw_message': raw_json, 'bot_name': bot.name,
            })
        ...
        if et in MESSAGE_TYPES or et == INTERACTION_CREATE:
            ...
            self._push_web_log('message', {
                ...
                'appid': appid, 'bot_name': bot.name, 'bot_qq': getattr(bot, 'robot_qq', '') or '',
                'event_type': et,
            })
    ...
    async def run_phase(self, config: StressTestConfig) -> None:
        ...
        for scenario in self._scenarios:
            ...
            for rate in self._rates:
                ...
                print(f"    Scenario={scenario} Rate={rate}/s ...")
                ...
                if scenario == "with_dedup":
                    ...
                    # Interleave: unique, dup, unique, dup... to test hits inline
                    interleaved = []
