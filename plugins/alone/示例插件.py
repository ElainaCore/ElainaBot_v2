"""示例功能：媒体、流式消息、卡片、按钮、群管理、主动消息和网页面板。"""
...
async def send_local_image(event, match):
    image = await asyncio.to_thread((_PLUGIN_DIR / '1.png').read_bytes)
    await event.reply_image(image, '📸 本地图片')


@handler(r'^语音$', name='语音', desc='发送语音', owner_only=True)
async def send_voice(event, match):
    await event.reply_voice('https://act-upload.mihoyo.com/sr-wiki/2025/06/03/160045374/420e9ac5c0c9d2b2c44b91f453b65061_2267222992827173477.wav')
...
async def send_file(event, match):
    await event.reply_file(str(_PLUGIN_DIR / '1.txt'), '📄 文件示例', file_name='test.txt')
...
async def send_ark23(event, match):
    await event.reply_ark(23, ('列表卡片示例', 'ElainaBot', [['功能1: 图片'], ['功能2: 语音'], ['功能3: 视频', 'https://i.elaina.vin/api/']]))


@handler(r'^ark24$', name='ark24', desc='文本+图片卡片', owner_only=True)
async def send_ark24(event, match):
    await event.reply_ark(24, ('功能强大的QQ机器人', '机器人信息', 'ElainaBot', '支持插件化开发', IMG, 'https://i.elaina.vin/api/', 'QQ Bot'))


@handler(r'^ark37$', name='ark37', desc='大图文卡片', owner_only=True)
async def send_ark37(event, match):
    await event.reply_ark(37, ('系统通知', '状态更新', '新功能上线', IMG, 'https://i.elaina.vin/api/'))


@handler(r'^图文卡片$', name='图文卡片', desc='card 卡片消息 (tuwen)', owner_only=True)
async def send_card(event, match):
    # 图文卡片元组依次为标题、描述、图片地址和跳转地址
    await event.reply_card('tuwen', ('QQ开放平台', '2分钟完成注册并创建QQBot', IMG, 'https://q.qq.com/#/'))
    # 也可以传入自定义卡片类型和字段字典，作为卡片内容原样发送
...
BUTTONS = [
    # 类型：0 为跳转链接，1 为回调，2 为输入指令，4 为订阅
    [{'text': '点我回调', 'data': 'callback_1', 'type': 1}, {'text': '输入框', 'data': '/帮助', 'type': 2}],
    [{'text': '打开链接', 'link': 'https://i.elaina.vin/'}],
    # 样式：0 灰框，1 蓝框蓝字，2 黑框，3 黑框红字，4 蓝底白字
    [
    ...
    [
        {
            'text': '订阅',
            'show': '已订阅',
            'subscribe': '102134274_1749040268',  # 替换为机器人的富文本模板标识
...
async def send_subscribe_message(event, match):
    ...
    ok, data, _ = await event.send_to_group(group_id, '🔔 这是一条订阅消息推送', subscribe_id=t['subscribe_id'])
...
async def query_share_count(event, match):
    ...
    lines += [f'{i}. {mask_id(oid)}  来源：{ls.get_scene_name(scene)}' for i, (oid, scene) in enumerate(refs.items(), 1)]
    await event.reply('\n'.join(lines))
...
async def query_my_sharer(event, match):
    ...
    await event.reply(f'📌 分享者：{mask_id(sid)}' if sid else '📌 你不是通过邀请链接添加的')
...
async def query_group_member(event, match):
    ...
    await event.reply(f'👤 群昵称: {member.get("username", "")}\n🎖️ 身份: {member.get("member_role", "")}\n📅 入群时间: {member.get("joined_at", "")}')
...
async def query_bot_member(event, match):
    ...
    await event.reply(f'🤖 群昵称: {member.get("username", "")}\n🎖️ 身份: {role}\n👮 是否管理员: {"是" if role in ("admin", "owner") else "否"}')
...
# ==================== 群管理 ====================
# 查询接口每分钟最多调用 30 次，修改接口每分钟最多调用 60 次
...
async def query_group_mute(event, match):
    ...
    await event.reply(f'全员禁言：{mode.get(setting["global_rule"]["mode"], "未知")}\n被禁言成员：{names}')
...
async def mute_group_member(event, match):
    ...
    success, error = await event.sender.set_group_member_mute(
        event.group_id,
        [
            {
                'op': 'add',
...
async def wakeup_user(event, match):
    ...
    await event.reply(f'✅ 已向 {mask_id(uid)} 发送召回')
...
async def force_wakeup_user(event, match):
    ...
    await event.reply(f'✅ 已强制召回 {mask_id(uid)}')
...
async def test_send_to_user(event, match):
    ...
    await event.reply(f'✅ 已发送给 {mask_id(uid)}')
...
async def test_send_to_group(event, match):
    ...
    await event.reply(f'✅ 已发送到群 {mask_id(gid)}')


@handler(r'^无event群发\s+(\S+)\s+(.+)$', name='无event主动群发', desc='不借助 event, 直接用 sender 推送', owner_only=True)
async def send_without_event(event, match):
    # 定时任务等没有事件对象的场景，可以直接取得消息发送器
    from core.bot.manager import _bot_manager_ref
...
register_page(
    ...
    source='plugin',
    source_name='alone',
    html='<h1>自定义侧边栏页面</h1><p>由 plugins/alone/示例插件.py 注册，插件卸载时自动注销。</p>',
)
...
# ==================== 优先级拦截 ====================
# 同一指令有多个处理器时，高优先级处理器可以阻止低优先级处理器继续执行
