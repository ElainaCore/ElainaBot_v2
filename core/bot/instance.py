...
class BotInstance:
    ...
    async def start(self, on_event):
        ...
        api_info = (
            f", API={self.sender._base_url}" if self.sender._custom_api_base else ""
        )
        bot_log.info(
            f"✅ 启动完成 (WS={'启用' if self.ws_client else '禁用'}{api_info})"
        )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if name:
                    ...
                    return
            get_logger(FRAMEWORK, self.appid).warning(
                "获取机器人昵称失败, 使用 appid 代替"
            )
        except Exception as e:
            get_logger(FRAMEWORK, self.appid).warning(
                f"获取机器人昵称异常: {e}, 使用 appid 代替"
            )
