...
class MessageSender(_HttpMixin, _MediaSendMixin):
    ...
    async def reply(
        ...
        **kwargs,
        ...
        if template_name:
            ...
            content, tpl_buttons = tpl.render(
                template_name, use_markdown=use_md, appid=self._appid, **vars_
            )
        ...
        success, data = await self._send_with_error_handling(
            endpoint, payload, event, content
        )
        if success:
        ...
        *,
        ...
        if isinstance(file_data, str) and file_data.startswith(("http://", "https://")):
            file_info = await upload_media_via_url(
                self, event, file_data, 4, file_name=file_name, **kw
            )
            return (
                await self._send_media_payload(event, file_info, content, **kw)
                if file_info
                else None
            )
        # 本地路径 → 异步读取
        ...
        return await self._send_media(
            event, file_data, 4, content, file_name=file_name, **kw
        )

    async def reply_ark(
        self, event, template_id, kv_data, content="", *, auto_delete_time=None
    ):
    ...
    async def send_to_channel(
        self, channel_id, content=None, *, msg_id=None, buttons=None, **kwargs
    ):
    ...
    async def upload_media(self, event, file_bytes, file_type, *, file_name=None):
        ...
        return await upload_media_bytes(
            self, file_bytes, file_type, endpoint, file_name=file_name
        )

    # ==================== 载荷构建 ====================

    def _build_payload(
        self, event, content, buttons, media, msg_type, *, prompt_buttons=None, **kwargs
    ):
    ...
    def _build_core_payload(self, content, buttons, media, msg_type, **kwargs):
        ...
        if media:
            ...
            suffix = (
                ""
                if skip_suffix
                else cfg.get_bot_setting(self._appid, "message.markdown_suffix", "")
            )
            payload["markdown"] = {
                "content": md_content + suffix if suffix else md_content
            }
        else:
    ...
    async def _send_with_error_handling(
        self, endpoint, payload, event, content=None, *, media_label=""
    ):
        ...
        if not success:
            ...
            report_error_raw(
                FRAMEWORK,
                "消息发送",
                content=json.dumps(raw_event, ensure_ascii=False, default=str)
                if raw_event
                else (getattr(event, "content", "") or ""),
    ...
    def _log_sent(self, payload, event, content, media_label="", resp_data=None):
        ...
        plugin_name = (
            getattr(event, "_reply_plugin_name", "") or self._reply_plugin_name
        )
