...
async def upload_media_bytes(
    sender, file_bytes, file_type, endpoint, *, file_name=None
):
    ...
    if len(file_bytes) > CHUNK_THRESHOLD:
        ...
        for retry in range(3):
            try:
                result = await _chunked_upload_from_bytes(
                    sender, file_bytes, file_type, endpoint, file_name=file_name
                )
                if result:
                    return result
    ...
    log.warning(
        f"[{sender._appid}] 上传失败: 无 file_info (endpoint={endpoint}, resp={last_resp})"
    )
    return None
...
async def _chunked_upload_from_bytes(
    sender, file_bytes, file_type, endpoint, *, file_name=None
):
    """内存数据分片上传 (写临时文件)"""
...
async def chunked_upload(sender, file_path, file_type, endpoint, *, file_name=None):
    ...
    hashes = await asyncio.get_running_loop().run_in_executor(
        None, compute_file_hashes, file_path, file_size
    )
    ...
    for prep_retry in range(3):
        ...
        if success:
            break
        log.warning(
            f"[{sender._appid}] upload_prepare 第{prep_retry + 1}次失败: {prep}"
        )
        if prep_retry < 2:
    ...
    with open(file_path, "rb") as f:
        while chunk := f.read(65536):
            ...
            if need_10m:
                ...
                if remaining > 0:
                    md5_10m_h.update(
                        chunk[:remaining] if remaining < len(chunk) else chunk
                    )
