...
async def upload_media_via_url(sender, event, url, file_type, *, file_name=None,
...
async def _chunked_upload_from_bytes(sender, file_bytes, file_type, endpoint, *, file_name=None):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as f:
            f.write(file_bytes)
            tmp_path = f.name
        return await chunked_upload(sender, tmp_path, file_type, endpoint,
...
async def chunked_upload(sender, file_path, file_type, endpoint, *, file_name=None):
    ...
    for part in parts:
        ...
        await sender.post_json(f"{scope}/upload_part_finish", {
            ...
            'block_size': len(chunk), 'md5': hashlib.md5(chunk).hexdigest()
        })
